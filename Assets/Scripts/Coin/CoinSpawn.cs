...
{ public GameObject coinPrefab;
    ...
    {
        Transform exitPoint = tile.transform.Find("CoinStartPoint");

        Transform startPos = tile.transform.Find("CoinExitPoint");
        Vector3 endPos = exitPoint.position;

        Vector3 direction = (endPos - startPos.position).normalized;
        float tileLength = Vector3.Distance(startPos.position, endPos);
        ...
        for (int i = 1; i <= coinsPerTile; i++)
        {
            ...
            Vector3 spawnPos = startPos.position + direction * distance;
